...
export default function MyBookingsClient() {
  ...
  const loadBookings = async (e: FormEvent) => {
    ...
    setSearched(true);
    try {
      const res = await fetch(`/api/my-bookings?phone=${encodeURIComponent(digits)}`);
    ...
    <div className="min-h-screen bg-black">
      <div className="max-w-4xl mx-auto px-4 py-8 sm:py-12">
        <div className="mb-6 sm:mb-8">
          <AppBackLink href="/" label="Back to home" className="mb-4" />
          <h1 className="text-3xl sm:text-4xl font-bold text-white mb-2">My Bookings</h1>
          <p className="text-gray-400">Enter your phone number to view reservations</p>
        </div>
        ...
        {!loading && searched && bookings.length === 0 && (
          <div className="bg-gradient-to-br from-gray-900 to-gray-800 rounded-2xl p-8 sm:p-12 text-center border border-white/10">
            ...
            <p className="text-gray-400">No reservations found for this phone number.</p>
          </div>
        )}
