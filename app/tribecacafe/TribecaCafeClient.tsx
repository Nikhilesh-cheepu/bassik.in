...
export default function TribecaCafeClient() {
  ...
  const patchChannel = async (id: string, status: string) => {
    setBusyId(id);
    try {
      const res = await fetch("/api/tribecacafe/channels", {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ id, status }),
      });
  ...
  const advanceCampaign = async (id: string) => {
    setBusyId(id);
    try {
      const res = await fetch("/api/tribecacafe/campaigns", {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
  ...
  const addEvent = async (e: React.FormEvent) => {
    ...
    setBusyId("event-new");
    try {
      const res = await fetch("/api/tribecacafe/events", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          monthId: month.id,
  ...
  const addRequest = async (e: React.FormEvent) => {
    ...
    setBusyId("req-new");
    try {
      const res = await fetch("/api/tribecacafe/requests", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
