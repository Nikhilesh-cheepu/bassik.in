# Booking Page — UI & Flow (Copy-Paste Prompt)

Use the following as a prompt for building the booking UI and flow. No backend or database details — only what the user sees and what happens when they interact.
...
- Fetch or compute the list of **offers valid for the selected date and selected time**. Each offer has: title, optional description, optional time-window label (e.g. "12PM – 7PM"), and optionally “Limited slots” / “Selling out fast” / “Few slots left” or “SOLD OUT” (no exact numbers).
- **Offer visibility rule:** An offer is shown only if the **selected time** falls inside that offer’s time window. Example: if the offer is valid 12:00–19:00 and the user selected 2:30 PM, show it; if the user selected 10:30 PM, do not show it.
